...
class Ui_ventanaAcceso(QDialog):
	...
	def validacionScan(self):
		...
		textLimpio = dencriptador.cleanCharQR( text )
    	readDatos = dencriptador.decodeInfoQR( textLimpio )
		print(readDatos)
		inicioChar=readDatos[:1]
	...
	def scan(datosScaned):
		global leido
		#thread3 = Thread(target=leerCodQR, args = ())
		#text=self.lscan.text()
		
		text2char=datosScaned[:2]
		if('M,' == text2char or 'L,' == text2char):
			if 'M,' == text2char:
				datosScaned = datosScaned[0:-1:]   #Retira caracteres de inicio y de fin
			#text=text.replace("'","-")
			#text=text.replace("Ñ",":")
			#text=text.split(',')
			#os.system("sudo nice -n -19 python3 archimp.py")
			#os.system("sudo nice -n -19 python3 archimp.py")
			try:
				print(str(datosScaned[3])+" "+str(datosScaned[4]),'datetime...')
				fecha = datetime.strptime(str(datosScaned[3])+" "+str(datosScaned[4]), '%d-%m-%Y %H:%M:%S')
				leerArch = open("/home/cajero/Documentos/ticket.txt", "w")
				leerArch.write(str(datosScaned[0])+"\n"+str(datosScaned[1])+"\n"+str(datosScaned[2])+"\n"+str(datosScaned[3])+"\n"+str(datosScaned[4])[:8])
				leerArch.close()
				textStr = ''
				for i in datosScaned:
					textStr = textStr + i + ","
				leido = textStr
				self.lscan.setText('')
				print('leido...',leido)
			...
			mensajeBoletoUsado = 1
			self.lscan.setText('')
			print('boleto invalido',datosScaned,text2char)	
