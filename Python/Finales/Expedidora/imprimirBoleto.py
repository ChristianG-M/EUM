...
def imprimirQR22(noBol,terminalEnt,fechaIn,horaEnt,plaza,localidad,pol):
	...
	try:
		...
		'''DL17'''
		encriptador = codificar()		
		encriptar = encriptador.codeInfoQR(content)
		CRC = encriptador.calcularCR7(encriptar)
		QRFinal = encriptar + bytes(',', 'utf-8') + bytes(CRC, 'utf-8') + bytes('.', 'utf-8')
		'''DL17'''
		Generic.set(align=u'center',font=u'B')
		Generic.qr(QRFinal,ec=0,size=4)	
